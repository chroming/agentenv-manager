  ...
  return (
    <code className={className}>
      {lines.map((line, lineIndex) => (
        <span className="document-markdown__code-line" key={lineIndex}>
...
const ExternalMarkdownLink = ({
  ...
  if (!href || !/^https?:\/\//i.test(href)) {
    return <span className="document-markdown__link-label">{children}</span>;
  }
  return (
    <a
      href={href}
...
export const ReadOnlyMarkdown = ({
  ...
  <div className={`document-markdown${wrapCode ? " document-markdown--wrap" : ""}`}>
    <ReactMarkdown
      ...
      components={{
        ...
        img: ({ alt, src }) => (
          <span
            className="document-markdown__image-reference"
            title={src}
          >
            {alt || src || "Image"}
          </span>
