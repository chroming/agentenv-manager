  ...
  return {
    ...
    async generate(raw: SkillSummaryGenerateInput): Promise<SkillSummary> {
      ...
      try {
        ...
        const categoryRules = 'Category is a machine-readable enum: exactly one of ["important", "usage", "security", "other"]. Never translate category values or invent categories. Classify breaking changes as "important", compatibility/setup requirements as "usage", and concrete security exposure as "security". Only overview, fact and implication use the requested language. Valid example: {"overview":"Workflow changed", "items":[{"category":"important", "fact":"A required input changed", "implication":"Update the input before use", "paths":["SKILL.md"]}]}. The example is format only; report only evidence from the supplied files.';
        const response = await request({ ...config, content, system: `${prompt}\n${categoryRules}`, signal: abort.signal });
