  ...
  return {
    ...
    async generate(raw: SkillSummaryGenerateInput): Promise<SkillSummary> {
      ...
      try {
        ...
        const content = JSON.stringify({ ...payload, context, allowedEvidencePaths: snapshot.files.map((file) => file.path), files: requestFiles });
        ...
        const categoryRules = 'Category is a machine-readable enum: exactly one of ["important", "usage", "security", "other"]. Never translate category values or invent categories. Classify breaking changes as "important", compatibility/setup requirements as "usage", and concrete security exposure as "security". Only overview, fact and implication use the requested language.';
        const evidenceRules = 'For every finding, copy paths verbatim from allowedEvidencePaths. Only files in this list carry analyzed evidence. Never cite omittedPaths, context-only files, directory names, URLs, diff a/ or b/ prefixes, line numbers or Markdown links. A file listed in changes is not citable unless it is also in allowedEvidencePaths. Do not invent a supporting path; omit a finding if no supplied excerpt supports it. Before returning, check every paths entry against allowedEvidencePaths.';
        const briefRules = "Use a decision brief: overview is one conclusion, at most 25 English words or 45 Chinese characters. Ordinary findings together should stay under 100 English words or 160 Chinese characters; retain additional distinct concrete risks even if that exceeds this target. Merge repeated causes. Do not repeat the overview in findings or repeat fact in implication. Do not add generic advice, reassurance, filenames or version numbers as prose filler. Each finding must explain a behavioral difference or concrete risk the user can act on.";
        const response = await request({ ...config, content, system: `${prompt}\n${categoryRules}\n${evidenceRules}\n${briefRules}`, signal: abort.signal });
