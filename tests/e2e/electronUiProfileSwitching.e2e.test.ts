...
const launchApp = async (
  options: {
    ...
    emptyOpenCodeProfileInstructions?: boolean;
  ...
  if (!options.omitAllProfiles && !options.omitOpenCodeProfiles) {
    ...
    if (options.emptyOpenCodeProfileInstructions) {
      await writeFile(
        join(appDataRoot, "profiles", "ui-opencode-alpha", "INSTRUCTIONS.md"),
        "",
        "utf8"
      );
    }
  ...
  if (options.skillGroupFixture) {
    ...
    await writeJson(join(appDataRoot, "skill-groups.json"), {
      ...
      groups: [{
        ...
        id: "review-pack",
        name: "Review pack",
        description: "Profile Group fixture",
        skillIds: memberIds,
    ...
    await writeJson(resourcesPath, {
      ...
      skills: [
        ...resources.skills.map((skill) => ({ ...skill, direct: false, groupIds: [groupId] })),
        {
          libraryId,
...
const selectTarget = async (page: Page, name: string) => {
  ...
  const targetSwitcher = page.getByRole("dialog", { name: "Select apply Agent" });
  await targetSwitcher.getByRole("option", { name, exact: true }).click();
  await targetSwitcher.waitFor({ state: "hidden" });
};
...
const editProfileInstructions = async (page: Page, value: string) => {
  await expandComposerSection(page, "Instructions");
  const profileName = (await page
    .locator(".profile-switcher--hero .ui-object-switcher__trigger-title")
    .textContent())?.trim();
  if (!profileName) throw new Error("The selected Profile name is unavailable");
  await page.getByRole("button", { name: `${profileName} instructions`, exact: true }).click();
  const dialog = page.getByRole("dialog", { name: "Edit Instruction Block" });
  await dialog.waitFor({ state: "visible" });
  const editor = dialog.getByRole("textbox", { name: "Instruction content" });
  ...
  expect(await preview.evaluate((element: HTMLElement) => ({
    contained: element.scrollWidth <= element.clientWidth + 1,
    overflowX: getComputedStyle(element).overflowX
...
const readMigratedProfileInstruction = async (
  appDataRoot: string,
  profileId: string
) => {
  const resources = await readJson<{
    instructions?: Array<{ libraryId: string; enabled: boolean }>;
  }>(join(appDataRoot, "profiles", profileId, "resources.json"));
  const libraryId = resources.instructions?.[0]?.libraryId;
  if (!libraryId) throw new Error(`Profile ${profileId} has no Instruction Library reference`);
  return readFile(join(appDataRoot, "instructions-library", libraryId, "CONTENT.md"), "utf8");
};
...
const openSharedSkills = async (page: Page) => {
  ...
  const localSkills = page.getByRole("region", { name: "Local Skills Manager" });
  await localSkills.getByRole("button", { name: "Review shared folder" }).click();
  await page.getByRole("region", { name: "Shared Skills" }).waitFor({ state: "visible" });
};
...
type SettingsCategoryName = "General" | "Skills" | "Conversations" | "Connections" | "Data";
...
const saveProfile = async (page: Page) => {
  ...
  await expect.poll(
    () => page.locator(".profile-row__dirty").count(),
    { timeout: 10_000 }
  ).toBe(0);
  await expect.poll(
...
describe("Electron UI profile switching e2e", () => {
  ...
  it("keeps quiet Agent identities and device navigation readable across locales and widths", async () => {
    ...
    for (const locale of ["en", "zh_CN", "zh_TW"]) {
      ...
      await page.locator('[data-workspace="targets"]').click();
      for (const width of [920, 1180, 1440]) {
        ...
        const geometry = await page.locator(".target-list").evaluate((list) => {
          const rangeLeft = (element: Element) => {
            const range = document.createRange();
            range.selectNodeContents(element);
            return range.getBoundingClientRect().left;
          };
          const heading = list.querySelector(".target-list__header")!;
          const headings = heading.querySelectorAll(":scope > span");
          const rows = [...list.querySelectorAll<HTMLElement>(".target-workflow-header")];
          return {
            columns: headings.length,
            contained: rows.every((row) => row.scrollWidth <= row.clientWidth + 1),
            identityAligned: rows.every((row) => Math.abs(rangeLeft(row.querySelector(".target-workflow-name-action strong")!) - rangeLeft(headings[1]!)) <= 1),
            profileAligned: rows.every((row) => Math.abs(row.querySelector(".target-workflow-environment")!.getBoundingClientRect().left - rangeLeft(headings[2]!)) <= 1),
            healthBesideIdentity: rows.every((row) => Boolean(row.querySelector(".target-workflow-name-line .target-health-status"))),
            summaryContained: (() => {
              const summary = document.querySelector<HTMLElement>(".sidebar-agent-summary__label")!;
              return summary.scrollWidth <= summary.clientWidth + 1;
            })()
          };
        });
        expect(geometry).toEqual({ columns: 3, contained: true, identityAligned: true, profileAligned: true, healthBesideIdentity: true, summaryContained: true });
        if (process.env.AGENTENV_LIBRARY_LAYOUT_CAPTURE_DIR) {
          ...
          await page.screenshot({ path: join(process.env.AGENTENV_LIBRARY_LAYOUT_CAPTURE_DIR, `quiet-agents-${locale}-${width}.png`) });
        }
      }
    }
  }, standardElectronTestTimeout);
    ...
    for (const width of [920, 1180, 1440]) {
      ...
      for (const [workspace, toolbar] of [
        ...
        if (workspace === "targets") {
          ...
          const name = controls.locator(":scope > span").nth(2);
  ...
  it.each(["en", "zh_CN"] as const)("keeps object workspaces to one header and stable resource commands in %s", async (locale) => {
    ...
    for (const width of [920, 1180, 1440]) {
      await resizeAppWindow(page, width, width === 920 ? 620 : 900);
      for (const workspace of ["profiles", "projects"] as const) {
        await page.locator(`.workspace-button[data-workspace="${workspace}"]`).click();
        ...
        const geometry = await header.evaluate((element) => {
          const box = element.getBoundingClientRect();
          const identity = element.querySelector(".ui-inspector-header__identity")!.getBoundingClientRect();
          const actions = element.querySelector(".ui-inspector-header__actions")!.getBoundingClientRect();
          return { noOverlap: identity.right <= actions.left + 1, contained: actions.right <= box.right + 1 && actions.bottom <= box.bottom + 1, overflow: element.scrollWidth > element.clientWidth };
        });
        expect(geometry).toEqual({ noOverlap: true, contained: true, overflow: false });
        const sections = page.locator(workspace === "profiles" ? ".profile-composer > .ui-resource-disclosure" : ".project-resource-section");
        await sections.first().waitFor();
        const summaryOrigins = () => sections.locator(":scope > header .ui-resource-disclosure__summary").evaluateAll((nodes) => nodes.map((node) => node.getBoundingClientRect().right));
        const before = await summaryOrigins();
        ...
        for (let index = 0; index < 3; index += 1) {
          ...
          if (workspace === "profiles" || index === 1) expect(await actions.count()).toBeGreaterThan(0);
          for (const action of await actions.all()) {
            await expectInViewport(page, action);
            expect(await action.evaluate((node) => {
              const style = getComputedStyle(node);
              return { border: style.borderBottomWidth, top: style.paddingTop, bottom: style.paddingBottom };
            })).toEqual({ border: "0px", top: "0px", bottom: "0px" });
          }
          await page.mouse.move(2, 2);
          if (captureDir && index === 1) await page.screenshot({ animations: "disabled", path: join(captureDir, `object-${workspace}-${locale}-${width}.png`) });
        }
        for (let index = 0; index < 3; index += 1) {
          const trigger = sections.nth(index).locator(":scope > header > button");
          if (await trigger.getAttribute("aria-expanded") === "true") await trigger.click();
    ...
    if (locale === "en") {
      ...
      await page.getByRole("dialog", { name: "Choose Workspace" }).getByRole("button", { name: "Add Workspace" }).click();
      expect(await page.getByRole("menuitem", { name: "Add local folder" }).count()).toBe(1);
      expect(await page.getByRole("menuitem", { name: "Add SSH remote workspace..." }).count()).toBe(1);
      await page.keyboard.press("Escape");
    }
  }, standardElectronTestTimeout);
  ...
  it("keeps configured Agent Profiles on one line before and after edits", async () => {
    ...
    for (const pending of [false, true]) {
      ...
      await page.locator(".target-workflow-profile").filter({ hasText: pending ? "Long Profile" : "UI OpenCode alpha" }).waitFor();
  ...
  it("refreshes Profiles with quiet contextual status and stable page geometry", async () => {
    ...
    const refresh = page.getByRole("button", { name: "More Profile actions" });
  ...
  it("keeps default catalog headers neutral without hiding configuration or update actions", async () => {
    ...
    for (const width of [920, 1180, 1440]) {
      ...
      const actionGeometry = await agents.locator(".target-workflow-environment").first().evaluate((cell) => {
        const action = cell.querySelector(".target-workflow-profile")!;
        return {
          textAligned: ["left", "start"].includes(getComputedStyle(action).textAlign),
  ...
  it("routes validated legacy ownership into reversible cleanup without changing deployment topology", async () => {
    ...
    await expect.poll(() => group.textContent()).toContain("Not managed");
  ...
  it("keeps unclaimed shared Skills quiet and exposes them from Local Skills", async () => {
    const { page } = await launchApp({
      initialWorkspace: null,
    ...
    expect(await agents.getByRole("button", { name: "Review Skills" }).count()).toBe(0);
    ...
    await openSkillLibrary(page);
    await openLocalSkills(page);
    const cleanup = page.getByRole("region", { name: "Local Skills Manager" });
    await cleanup.waitFor({ state: "visible" });
    await cleanup.getByRole("button", { name: "Review shared folder" }).click();
    const sharedCleanup = page.getByRole("region", { name: "Shared Skills" });
    await sharedCleanup.waitFor({ state: "visible" });
    await sharedCleanup.getByText("Shared Skills on this device", { exact: true }).waitFor({
      state: "visible"
    });
    await sharedCleanup.getByRole("group", {
  ...
  it("supports platform-correct desktop shortcuts in real workspaces", async () => {
    ...
    await editProfileInstructions(page, "# Shortcut E2E\n");
    await expect
      .poll(() => readMigratedProfileInstruction(appDataRoot, "ui-opencode-alpha"))
      .toContain("Shortcut E2E");
    const dirtySwitcher = await openProfileSwitcher(page);
    await dirtySwitcher.getByRole("button", { name: "New Profile", exact: true }).click();
    const newProfileName = page.getByRole("dialog", { name: "New Profile" }).getByLabel("Profile name");
    await newProfileName.fill("Shortcut remains local");
    await page.keyboard.press("Meta+s");
    expect(await newProfileName.evaluate((element) => document.activeElement === element)).toBe(true);
    await page.keyboard.press("Escape");
    await page.keyboard.press("Control+s");
    await page.waitForTimeout(50);
    expect(await page.getByRole("button", { name: "Save", exact: true }).count()).toBe(0);
    expect(await readMigratedProfileInstruction(appDataRoot, "ui-opencode-alpha"))
      .toContain("Shortcut E2E");
  ...
  it("prevents duplicate Meta saves while the Electron IPC is pending", async () => {
    ...
    await expandComposerSection(page, "Skills");
    await page.getByRole("region", { name: "Profile Skills" }).getByRole("switch", {
    ...
    for (const testCase of cases) {
      ...
      const libraryHeaderGeometry = await page.locator(".library-page-header").evaluate((header) => {
        ...
        return {
          ...
          actionsBesideTitle: actionsRect.left >= copyRect.right - 1,
          centerDelta: Math.abs(
            copyRect.top + copyRect.height / 2 - (actionsRect.top + actionsRect.height / 2)
          )
        };
      });
      ...
      for (const action of toolbarActions) {
        ...
        toolbarWidths.push(geometry.buttonWidth);
  ...
  it("shows target readiness from installed commands and writable local paths", async () => {
    ...
    await openCodeCard.getByRole("button", { name: "OpenCode", exact: true }).waitFor();
  ...
  it("updates target cards after taking over OpenCode", async () => {
    ...
    const openCodeCard = page.getByRole("article", { name: "Agent OpenCode" });
    await openCodeCard.waitFor({ state: "visible" });
    await expect.poll(() => openCodeCard.locator(".target-workflow-profile").getAttribute("title")).toBe("Applied");
  ...
  it("creates, edits, duplicates, and deletes profiles through the rendered app", async () => {
    ...
    await page.getByRole("menuitem", { name: "Edit Profile" }).click({ timeout: 5_000 });
  ...
  it("consolidates an existing target skill into the shared library", async () => {
    ...
    await expect.poll(() => cleanupGroup.textContent()).toContain("Not managed");
    await cleanupGroup
    ...
    let cleanupDialog = page.getByRole("dialog", { name: "Manage eligible local Skills" });
    ...
    await cleanupDialog.getByRole("button", { name: /Manage \d+ skills/ }).click();
    await expect
    ...
    await page.getByRole("button", { name: /Manage \d+ eligible Skills/ }).click();
    cleanupDialog = page.getByRole("dialog", { name: "Manage eligible local Skills" });
    await cleanupDialog.getByRole("button", { name: /Manage \d+ skills/ }).click();
    await page
  ...
  it("keeps the single-Profile workspace and temporary switcher contained at supported viewports", async () => {
    ...
    const expectShellContained = async () => {
      ...
      const heroAlignment = await header.evaluate((element) => {
        ...
        const style = getComputedStyle(element);
        return {
          actualInset: body.getBoundingClientRect().left - heroBox.left,
          expectedInset: Number.parseFloat(style.paddingLeft)
        };
      });
      expect(heroAlignment.actualInset).toBeCloseTo(heroAlignment.expectedInset, 0);
    };
    ...
    const inspectSwitcher = async () => {
      ...
      const selectedOption = switcher.getByRole("option", {
        name: "Profile UI OpenCode alpha",
        exact: true
      });
      const reorderContract = await selectedOption.evaluate((row) => {
        const identity = row.querySelector<HTMLElement>(".ui-selectable-row__identity")!;
        const handle = row.querySelector<HTMLElement>(".ui-object-switcher__drag-handle")!;
        const rowBox = row.getBoundingClientRect();
        const identityBox = identity.getBoundingClientRect();
        const handleBox = handle.getBoundingClientRect();
        return {
          handleDraggable: handle.getAttribute("draggable"),
          handleInsideRightEdge: handleBox.right <= rowBox.right && handleBox.left > identityBox.right,
          rowDraggable: row.getAttribute("draggable"),
          textSelection: getComputedStyle(identity).userSelect
        };
      });
      expect(reorderContract.rowDraggable).toBeNull();
      expect(reorderContract.handleDraggable).toBe("true");
      expect(reorderContract.handleInsideRightEdge).toBe(true);
      expect(reorderContract.textSelection).toBe("text");

      await selectedOption.evaluate((row) => {
        const identity = row.querySelector<HTMLElement>(".ui-selectable-row__identity")!;
        const range = document.createRange();
  ...
  it.each([
    ...
    for (const viewport of [
      ...
      await expectInViewport(page, page.locator(".system-status-card .sidebar-agent-summary"));
      expect(await page.locator(".system-status-card .sidebar-agent-summary__label").count()).toBe(1);
  ...
  it("keeps core management actions usable at the minimum supported viewport", async () => {
    ...
    const expectCommitActionsToFit = async () => {
      ...
      const targetSelector = commitActions.locator(".profile-agent-switcher .ui-object-switcher__trigger");
      const [
        titleBox,
        applyBox,
        targetSelectorBox,
      ...
      expect(moreButtonBox!.x).toBeGreaterThanOrEqual(
        applyButtonBox!.x + applyButtonBox!.width
      );
      expect(
        moreButtonBox!.x - (applyButtonBox!.x + applyButtonBox!.width)
      ).toBeLessThanOrEqual(10);
      expect(Math.abs(targetSelectorBox!.y - applyButtonBox!.y)).toBeLessThanOrEqual(1);
      expect(Math.abs(targetSelectorBox!.height - applyButtonBox!.height)).toBeLessThanOrEqual(1);
      expect(Math.abs(moreButtonBox!.height - applyButtonBox!.height)).toBeLessThanOrEqual(1);
      expect(Math.round(targetSelectorBox!.width)).toBe(150);
      expect(Math.round(applyButtonBox!.height)).toBe(32);
      ...
      const heroContent = await page.locator(".profile-hero").evaluate((hero) => ({
        description: hero.querySelector(".profile-description") !== null,
      ...
      if (heroContent.width <= 760) {
        expect(heroContent.description).toBe(false);
    ...
    await profileActionsMenu.getByRole("menuitem", { name: "Compare" }).waitFor({
      state: "visible"
    });
    expect(
    ...
    await expect
      .poll(() => commitActions.locator(".profile-apply-button").isEnabled())
      .toBe(true);
    await expectCommitActionsToFit();
  ...
  it("keeps global chrome and page control scale stable across primary workspaces", async () => {
    ...
    for (const viewport of [
      ...
      await expectInViewport(page, page.getByRole("button", { name: "Show Local Agents" }));
  ...
  it("compacts resource lanes semantically at the minimum desktop width", async () => {
    ...
    expect(targetLanes.columnCount).toBe(4);
  ...
  it("preserves shared control surfaces through disabled, busy, active and expanded states", async () => {
    ...
    expect(styles.disabledPrimary.background).toBe("rgb(245, 245, 247)");
    expect(styles.disabledPrimary.background).not.toBe(styles.primary.background);
    expect(styles.disabledPrimary.opacity).toBe("1");
  ...
  it("keeps visible control and status text inside its component contract", async () => {
    ...
    await previewDialog.getByRole("button", { name: "Review local Skills", exact: true }).click();
    const localSkillsManager = page.getByRole("region", { name: "Local Skills Manager" });
    await localSkillsManager.waitFor({ state: "visible" });
    await localSkillsManager.getByText("All local Skills on this device", { exact: true }).waitFor({
      state: "visible"
    });
    expect(await localSkillsManager.getByRole("combobox", { name: "Management scope" }).count())
      .toBe(0);
    await localSkillsManager.getByRole("button", { name: "Close library tool" }).click();
    await previewDialog.waitFor({ state: "visible" });
    ...
    await openLocalSkills(page);
    const cleanupDrawer = page.getByRole("region", { name: "Local Skills Manager" });
    await cleanupDrawer.waitFor({ state: "visible" });
    await expectTextContracts("Local Skills Manager at 920x620");
  ...
  it("lays out Agents as one ordered management list with on-demand diagnostics", async () => {
    ...
    const expectAgentRowGeometry = async () => {
      const geometry = await page.locator(".target-list").evaluate((list) => {
        ...
        return {
          ...
          healthBesideIdentity: rows.every((row) => row.querySelector(".target-workflow-name-line .target-health-status")),
          ...
          singleLineCentered: rows.every((row) => {
            const action = row.querySelector(".target-workflow-profile")?.getBoundingClientRect();
          ...
          typography: {
            ...
            lifecycle: getComputedStyle(first.querySelector<HTMLElement>(".target-workflow-profile")!).fontWeight,
    ...
    expect(wideHeaderGeometry.lastAppliedWidth).toBe(0);
  ...
  it("restores device-local Profile selection and Agent order after a renderer restart", async () => {
    const { page } = await launchApp();
    const persisted = await page.evaluate(async () => {
      const profiles = (await window.agentEnv.listProfiles()).filter((profile) => !profile.loadError);
      const targets = await window.agentEnv.listTargets();
      const selectedProfile = profiles.at(-1)!;
      const profileOrder = profiles.map((profile) => profile.id).reverse();
      const agentOrder = targets.map((target) => target.id).reverse();
      await window.agentEnv.updateUiState({
      ...
      return {
        ...
        profileOrder: profileOrder.map((id) => profiles.find((profile) => profile.id === id)!.name),
        agentOrder: agentOrder.map((id) => targets.find((target) => target.id === id)!.name)
      };
    });
    ...
    await page.getByRole("button", { name: "Profiles", exact: true }).click();
    await page.getByRole("heading", { name: persisted.selectedProfileName }).waitFor({
      state: "visible"
    });
    const switcher = await openProfileSwitcher(page);
  ...
  it("reorders Agents from the Agents workspace and keeps that order", async () => {
    const { page } = await launchApp();
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    const agents = page.getByRole("region", { name: "Agents", exact: true });
    await agents.waitFor({ state: "visible" });
    const rows = agents.locator(".target-card--workflow");
    const initialNames = await rows.locator(".target-workflow-name-action").allTextContents();
    expect(initialNames.length).toBeGreaterThan(1);
    await agents.getByRole("button", { name: "More Agent actions" }).click();
    await page.getByRole("menuitem", { name: "Reorder Agents" }).click();
    ...
    const handleGeometry = await firstHandle.evaluate((handle) => {
      const row = handle.closest<HTMLElement>(".target-workflow-header")!;
      const handleBox = handle.getBoundingClientRect();
      const rowBox = row.getBoundingClientRect();
      return {
        centered: Math.abs(
    ...
    await expect.poll(() =>
      rows.locator(".target-workflow-name-action").allTextContents()
  ...
  it("keeps Target application policies on Composer rows and persists them per Agent", async () => {
    ...
    const addSkill = page.locator('[data-profile-composer-id="skills"]').getByRole("button", { name: "Add Skills", exact: true });
  ...
  it("edits a shared Instruction from the default-collapsed Composer", async () => {
    const { appDataRoot, page } = await launchApp();
    await page.setViewportSize({ width: 1180, height: 728 });
    await selectProfile(page, "UI OpenCode alpha");
    ...
    const instructionContent = `# Updated through collapsed Composer\n${"unbroken-profile-rule-".repeat(32)}\n`;
    await editProfileInstructions(page, instructionContent);
    ...
    await expect.poll(() => applyButton.isEnabled(), { timeout: 10_000 }).toBe(true);
    ...
    const commitControlGeometry = await page
      .locator(
        ".profile-commit-actions .ui-object-switcher__trigger, .profile-commit-actions .profile-apply-button, .profile-commit-actions .profile-more-button"
      )
    ...
    expect(commitControlGeometry).toHaveLength(3);
    ...
    await expect(readMigratedProfileInstruction(appDataRoot, "ui-opencode-alpha"))
      .resolves.toBe(instructionContent);
  }, standardElectronTestTimeout);

  it("renders an empty Profile Instructions section as one compact, actionable row", async () => {
    const { page } = await launchApp({ emptyOpenCodeProfileInstructions: true });
    await resizeAppWindow(page, 920, 620);
    await selectProfile(page, "UI OpenCode alpha");
    await expandComposerSection(page, "Instructions");

    const section = page.locator('[data-profile-composer-id="instructions"]');
    const emptyState = section.getByRole("status");
    await emptyState.waitFor({ state: "visible" });
    expect(await emptyState.textContent()).toContain("No Instructions in this Profile");
    expect(await section.getByRole("button", { name: "Preview output" }).count()).toBe(0);
    await section.getByRole("button", { name: "Add Instruction Blocks" }).waitFor();
    await expectNoHorizontalOverflow(page, ['[data-profile-composer-id="instructions"]']);
    expect(Math.round((await emptyState.boundingBox())!.height)).toBe(50);

    if (process.env.AGENTENV_EMPTY_INSTRUCTIONS_CAPTURE_DIR) {
      await mkdir(process.env.AGENTENV_EMPTY_INSTRUCTIONS_CAPTURE_DIR, { recursive: true });
      await page.screenshot({
        path: join(
          process.env.AGENTENV_EMPTY_INSTRUCTIONS_CAPTURE_DIR,
          "profile-empty-instructions-920x620.png"
        )
      });
    }
  }, standardElectronTestTimeout);

  it("persists a shared Instruction before leaving its Profile workspace", async () => {
    const { appDataRoot, page } = await launchApp();
    const savedDraft = "# Saved before navigation\n";
    const navigation = page.getByRole("complementary", { name: "Global navigation" });

    await selectProfile(page, "UI OpenCode alpha");
    await editProfileInstructions(page, savedDraft);
    await navigation.getByRole("button", { name: "Skills", exact: true }).click();
    await page.getByRole("heading", { name: "Skills" }).waitFor({
      state: "visible",
      timeout: 5_000
    });
    expect(await page.getByRole("dialog", { name: "Unsaved changes" }).count()).toBe(0);
    await expect(readMigratedProfileInstruction(appDataRoot, "ui-opencode-alpha"))
      .resolves.toBe(savedDraft);
  }, standardElectronTestTimeout);

  it("cancels a reverted Profile Skill edit before persistence", async () => {
    const { appDataRoot, page } = await launchApp();
    await selectProfile(page, "UI OpenCode alpha");
    await previewAndApply(page, "OpenCode");
    await expandComposerSection(page, "Skills");
  ...
  it("restores the selected Agent's last applied Profile version without changing the Agent", async () => {
    const { appDataRoot, opencodeDir, page } = await launchApp();
    const targetInstructionsPath = join(opencodeDir, "AGENTS.md");

    await selectProfile(page, "UI OpenCode alpha");
    await previewAndApply(page, "OpenCode");
    const originalTargetInstructions = await readFile(targetInstructionsPath, "utf8");
    await expandComposerSection(page, "Skills");
    await page.getByRole("region", { name: "Profile Skills" }).getByRole("switch", {
      name: "Disable ui-alpha-skill"
    }).click();
    await expect.poll(async () => {
      ...
      return resources.skills[0]?.enabled;
    }).toBe(false);

    await page.getByRole("button", { name: "More Profile actions" }).click();
    const undo = page.getByRole("menuitem", { name: "Restore last applied" });
    await expect.poll(() => undo.isEnabled()).toBe(true);
    await undo.click();
    const recovery = page.getByRole("dialog", { name: "Restore last applied Profile" });
    await recovery.getByRole("button", { name: "Restore Profile" }).click();
    ...
    await expect(readFile(targetInstructionsPath, "utf8")).resolves.toBe(originalTargetInstructions);
  }, standardElectronTestTimeout);
  ...
  it("waits for Profile resource auto-save before closing Electron", async () => {
    ...
    expect(resources.skills[0]?.enabled).toBe(false);
  }, standardElectronTestTimeout);
  ...
  it("imports a local skill folder from the Import dialog", async () => {
    ...
    await expect.poll(() => importedRow.textContent()).toContain("Local");
  ...
  it("surfaces legacy AgentEnv-owned shared links as an unfinished location migration", async () => {
    ...
    await cleanupGroup.waitFor({ state: "visible" });
    await expect.poll(() => cleanupGroup.textContent()).toContain("Not managed");
    await expect.poll(() => cleanupGroup.textContent()).toContain("Shared across Agents");
    await expect(
  ...
  it("imports and manages a shared Skill in place without changing Profiles", async () => {
    ...
    await page.getByRole("button", { name: /Manage \d+ eligible Skills/ }).click();
    const addSharedDialog = page.getByRole("dialog", { name: "Manage eligible local Skills" });
    await addSharedDialog.getByRole("button", { name: /Manage \d+ skills/ }).click();
    await expect
      .poll(
      ...
      .toBe(true);
    const closeManagement = addSharedDialog.getByRole("button", { name: "Close" });
    await expect.poll(() => closeManagement.isEnabled(), { timeout: 10_000 }).toBe(true);
    expect(sharedSkillStatsBefore.ino).toBeGreaterThan(0);
    await expect.poll(() => fileExists(sharedSkillDir), { timeout: 5_000 }).toBe(true);
    expect((await stat(join(sharedSkillDir, "SKILL.md"))).ino).toBe(sharedSkillStatsBefore.ino);
    await expect(fileExists(join(opencodeDir, "skills", "shared-migration-reviewer"))).resolves.toBe(false);
    await expect(fileExists(join(codexDir, "skills", "shared-migration-reviewer"))).resolves.toBe(false);
    await expect(readJson<{ mode: string; receipts: Array<{ path: string; libraryId: string }> }>(
      join(appDataRoot, "shared-skill-area.json")
    )).resolves.toMatchObject({
      mode: "managed",
      receipts: [{ path: sharedSkillDir, libraryId: "shared-migration-reviewer" }]
    });
  ...
  it("atomically migrates and restores a shared Skill after every consumer is prepared", async () => {
    ...
    await expect(preparationPreview.getByRole("button", {
    ...
    await expect(page.getByText("Shared copies prevent this Profile change").count())
      .resolves.toBe(0);
    const preparedSkillRow = page.getByRole("listitem", {
      name: `Profile Skill ${skillId}`
    });
    await expect.poll(() => preparedSkillRow.textContent()).not.toContain("Apply pending");
    await expect.poll(() => preparedSkillRow.textContent()).toContain("Will be on");
    await expect.poll(() => preparedSkillRow.getByRole("switch").isEnabled()).toBe(true);
    await selectTarget(page, "Codex");
    ...
    await openSharedSkills(page);
    const sharedSkillsArea = page.getByRole("region", { name: "Shared Skills" });
    ...
    for (const viewport of [
      ...
      await resizeAppWindow(page, viewport.width, viewport.height);
      const geometry = await page.locator(".shared-area-mode-actions").evaluate((actions) => {
        const box = actions.getBoundingClientRect();
        return {
          contained: actions.scrollWidth <= actions.clientWidth + 1,
          buttonsFit: Array.from(actions.querySelectorAll<HTMLElement>("button"))
            .every((button) => button.scrollWidth <= button.clientWidth + 1),
          visible: box.width > 0 && box.height > 0
        };
      });
      expect(geometry).toEqual({ contained: true, buttonsFit: true, visible: true });
    }
    await sharedSkillsArea.getByRole("button", { name: "Change…" }).click();
    await page.getByRole("button", {
      name: "Move shared Skills to Profile control…"
    }).click();
    const profilesOnlyDialog = page.getByRole("dialog", {
      name: "Move shared Skills to Profile control"
    });
    await expect.poll(() => profilesOnlyDialog.getAttribute("aria-label"))
      .toBe("Move shared Skills to Profile control");
    await expect.poll(() => profilesOnlyDialog.textContent()).toContain(skillId);
    await expect.poll(() => profilesOnlyDialog.textContent()).toContain(
      "Linked source folders and the parent shared folder are never deleted."
    );
    await expectInViewport(page, profilesOnlyDialog);
    const migrationCaptureDir = process.env.AGENTENV_SHARED_SKILL_CAPTURE_DIR;
    if (migrationCaptureDir) {
      await page.screenshot({
        path: join(migrationCaptureDir, "shared-migration-confirm-920x620.png")
      });
    }
    await profilesOnlyDialog.getByRole("button", {
      name: "Move shared Skills to Profile control"
    }).click();
    ...
    const closeProfilesOnlyDialog = profilesOnlyDialog.getByRole("button", { name: "Close" });
    await expect.poll(
      async () =>
        await closeProfilesOnlyDialog.count() > 0 &&
        await closeProfilesOnlyDialog.isEnabled({ timeout: 0 }),
      { timeout: 15_000 }
    ).toBe(true);
    await closeProfilesOnlyDialog.click();
    ...
    await sharedSkillsArea.getByRole("button", { name: "Change…" }).click();
    await page.getByRole("button", { name: "Restore shared setup…" }).click();
    await expect.poll(() =>
      migrationHistoryRow
  ...
  it("moves a shared Skill directly into an unmanaged Pi Environment from Scan local", async () => {
    ...
    await openLocalSkills(page);
    const localSkills = page.getByRole("region", { name: "Local Skills Manager" });
    await localSkills.getByRole("button", { name: "Refresh local skills" }).click();
    await expect.poll(() => localSkills.textContent()).toContain(skillId);
    await localSkills.getByRole("button", { name: "Close library tool" }).click();
    await openSharedSkills(page);
    const environmentSkills = page.getByRole("region", { name: "Shared Skills" });
    ...
    await environmentSkills.getByRole("button", { name: "Change…" }).click();
    await page.getByRole("button", {
      name: "Move shared Skills to Profile control…"
    }).click();
    ...
    await expect.poll(() => migrationDialog.getAttribute("aria-label"))
      .toBe("Move shared Skills to Profile control");
    ...
    await migrationDialog.getByRole("button", {
      name: "Move shared Skills to Profile control"
    }).click();
    ...
    const closeMigrationDialog = migrationDialog.getByRole("button", { name: "Close" });
    await expect.poll(
      async () =>
        await closeMigrationDialog.count() > 0 &&
        await closeMigrationDialog.isEnabled({ timeout: 0 }),
      { timeout: 15_000 }
    ).toBe(true);
    await closeMigrationDialog.click();
  ...
  it("auto-manages safe cleanup groups while leaving content conflicts for review", async () => {
    ...
    await expect.poll(() => safeGroup.textContent()).toContain("Not managed");
    await expect.poll(() => duplicateGroup.textContent()).toContain("Not managed");
    await expect.poll(() => sharedGroup.textContent()).toContain("Not managed");
    await expect.poll(() => brokenSharedGroup.textContent()).toContain("Unavailable");
    ...
    await expect.poll(() => page.getByRole("region", { name: "Local Skills Manager" }).count()).toBe(1);
    ...
    const assertCleanupLayout = async (stacked: boolean) => {
      const geometry = await page.getByRole("region", { name: "Local Skills Manager" }).evaluate((drawer) => {
    ...
    await page.getByRole("button", { name: /Manage \d+ eligible Skills/ }).click();
    const bulkCleanupDialog = page.getByRole("dialog", { name: "Manage eligible local Skills" });
    ...
    await expect.poll(() => bulkCleanupDialog.textContent()).toContain(
      "Save and manage existing copies"
    );
    ...
    await expect.poll(async () => {
      const state = await readJson<{
        mode?: string;
        receipts?: Array<{ path: string; libraryId: string }>;
      }>(join(appDataRoot, "shared-skill-area.json"));
      const receipt = state.receipts?.find((item) => item.path === sharedDuplicate);
      return {
        mode: state.mode,
        receipt: receipt
          ? { path: receipt.path, libraryId: receipt.libraryId }
          : undefined
      };
    }, { timeout: 10_000 }).toEqual({
      mode: "managed",
      receipt: {
        path: sharedDuplicate,
        libraryId: "auto-shared-reviewer"
      }
    });
    await expect(fileExists(targetSharedDuplicate)).resolves.toBe(false);
    await expect.poll(async () => {
      try {
    ...
    await expect.poll(
      () => bulkCleanupDialog.getByRole("button", { name: "Close" }).isEnabled(),
      { timeout: 15_000 }
    ).toBe(true);
    expect(await bulkCleanupDialog.getByText("Managed", { exact: true }).count())
      .toBeGreaterThan(0);
    await bulkCleanupDialog.getByRole("button", { name: "Close" }).click();
    await expect(
      fileExists(join(appDataRoot, "skills-library", "manual-conflict-reviewer"))
    ).resolves.toBe(false);
    await expect.poll(() => conflictGroup.textContent()).toContain("2 versions");
    ...
    await expect
      .poll(
        () => page.locator(".cleanup-group-row").evaluateAll((rows) =>
          rows
            .filter((row) => row.querySelector(".cleanup-group-state")?.textContent?.trim() === "Not managed")
    ...
    await expect
      .poll(
        () => page.getByRole("button", { name: /Manage \d+ eligible Skills/ }).count(),
        { timeout: 30_000 }
      )
      .toBe(0);
  }, 60_000);
  ...
  it("backs up Library drift, removes broken links, and leaves Environment membership unchanged", async () => {
    ...
    await driftDialog.getByRole("button", { name: "Apply cleanup" }).click();
    await driftDialog.waitFor({ state: "hidden" });
    await expect.poll(() => readFile(join(targetDir, "SKILL.md"), "utf8")).toContain(
      "# Library canonical"
    );
    ...
    await page.getByRole("button", { name: /Manage \d+ eligible Skills/ }).click();
    const dialog = page.getByRole("dialog", { name: "Manage eligible local Skills" });
    await expect.poll(() => dialog.textContent()).toContain("Remove unavailable links");
    await dialog.getByRole("button", { name: /Manage \d+ skills/ }).click();
  ...
  it("refreshes the open local skill cleanup with a new disk scan", async () => {
    ...
    await openSkillLibrary(page);
    await openLocalSkills(page);
    const drawer = page.getByRole("region", { name: "Local Skills Manager" });
    ...
    await expect.poll(() =>
      drawer.getByRole("button", { name: "Refresh local skills" }).getAttribute("aria-busy")
    ).toBe("false");
    expect(await page.getByText("Local skills refreshed", { exact: true }).count()).toBe(0);
  }, standardElectronTestTimeout);
  ...
  it("refreshes Skills in place without clearing the current view", async () => {
    ...
    expect(await page.getByText("Skills refreshed", { exact: true }).count()).toBe(0);
  ...
  it("persists custom Skill and Environment icons through the rendered app", async () => {
    ...
    await editProfileDialog
      .getByRole("button", { name: "Change icon for UI OpenCode alpha" })
      .click();
    await page
  ...
  it("detects and applies updates after a library skill is installed on OpenCode", async () => {
    ...
    await updateDialog.waitFor({ state: "visible" });
    await updateDialog
      .getByText(/Used by 1 Profiles.*1 copied Agent installs will need Apply/)
      .waitFor({ state: "visible" });
    ...
    expect(await updatedRow.getByText("1 Agent pending").count()).toBe(0);
  }, standardElectronTestTimeout);
  ...
  it("keeps capture actions visible with long, high-density review content", async () => {
      ...
      ipcMain.handle("profiles:preview-create-from-target", async () => {
        ...
        return {
          ...
          resources: [...Array.from({ length: 30 }, (_, index) => ({
            ...
            kind: "skill", id: "provided", name: "WorkBuddy", action: "observe", count: 2,
            observedSkills: [
              { name: "bundled-review-with-a-long-name", path: "/Applications/Example.app/skills/review", availability: "bundled" },
              { name: "connector-review", path: "/fixture/connectors/review", availability: "unknown" }
            ]
          }],
          warnings: Array.from(
            { length: 6 },
    ...
    const provided = dialog.getByRole("button", { name: "Provided by Agent" });
    for (const [width, height] of [[920, 620], [1180, 728], [1440, 900]]) {
      await resizeAppWindow(page, width, height);
      await provided.scrollIntoViewIfNeeded();
      await mkdir("/tmp/agentenv-capture-review", { recursive: true });
      await page.screenshot({ path: `/tmp/agentenv-capture-review/collapsed-${width}.png` });
      await provided.click();
      await dialog.getByText("connector-review", { exact: true }).scrollIntoViewIfNeeded();
      await expectNoHorizontalOverflow(page);
      await expectInViewport(page, dialog.getByRole("button", { name: "Save Profile" }));
      await page.screenshot({ path: `/tmp/agentenv-capture-review/expanded-${width}.png` });
      await provided.click();
    }
  ...
  it("reveals all sidebar Agents from the compact device summary", async () => {
    const { page } = await launchApp();
    const overflow = page.getByRole("button", {
      name: "Show Local Agents"
    });
    await overflow.waitFor({ state: "visible" });

    await overflow.click();
    const popover = page.getByRole("menu", { name: "Local Agents" });
    await popover.waitFor({ state: "visible" });
    ...
    expect(await popover.locator(".agent-chip--antigravity .agent-chip__logo").count()).toBe(2);
    ...
    for (const viewport of [
      ...
      await resizeAppWindow(page, viewport.width, viewport.height);
      if (!(await popover.isVisible())) await overflow.click();
    ...
    await page.locator(".shell-titlebar").hover();
    await popover.waitFor({ state: "hidden" });
  ...
  it("explains why an unavailable Agent cannot start Skill management", async () => {
    ...
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    await page.getByRole("button", { name: "Refresh" }).click();
    const antigravityCard = page.getByRole("article", {
      name: "Agent Antigravity CLI",
      exact: true
    });
  ...
  it("offers detected Agents without changing their files until the user enables one", async () => {
    ...
    await dialog.getByRole("button", { name: "Save Agent choices" }).click();
  ...
  it("recalibrates an older all-enabled Agent scope once and keeps missing Agents off", async () => {
    ...
    const workBuddyCheckbox = dialog.getByRole("checkbox", { name: "WorkBuddy" });
    expect(await workBuddyCheckbox.isChecked()).toBe(false);
    expect(await workBuddyCheckbox.locator("xpath=..").locator("img").getAttribute("src"))
      .toContain("workbuddy");
    if (process.env.AGENTENV_AGENT_DISCOVERY_CAPTURE_DIR) {
      await mkdir(process.env.AGENTENV_AGENT_DISCOVERY_CAPTURE_DIR, { recursive: true });
      await page.screenshot({
  ...
  it("persists enabled Agents and excludes disabled Agents from operations", async () => {
    ...
    await agentsWorkspace.getByRole("button", { name: "Choose Agents" }).click();
    const chooser = page.getByRole("dialog", { name: "Choose Agents" });
    ...
    for (const viewport of [
      ...
      await resizeAppWindow(page, viewport.width, viewport.height);
      await expectNoHorizontalOverflow(page, [".app-shell", ".editor-panel", ".agent-discovery-dialog"]);
      const rowGeometry = await chooser.locator(".agent-discovery-row").evaluateAll((rows) =>
        rows.map((row) => {
          const rowRect = row.getBoundingClientRect();
      ...
      expect(rowGeometry.length).toBeGreaterThanOrEqual(3);
      ...
      const visibleBoundary = await chooser.locator(".agent-discovery-list").evaluate((list) => {
        const listRect = list.getBoundingClientRect();
        const visibleRows = Array.from(list.querySelectorAll(".agent-discovery-row"))
          .map((row) => row.getBoundingClientRect())
          .filter((row) =>
            row.top < listRect.bottom - 1 && row.bottom > listRect.top + 1
          );
        return {
          listTop: listRect.top,
          listBottom: listRect.bottom,
          rows: visibleRows.map((row) => ({ top: row.top, bottom: row.bottom }))
        };
      });
      expect(visibleBoundary.rows.length).toBeGreaterThan(0);
      for (const row of visibleBoundary.rows) {
        expect(row.top).toBeGreaterThanOrEqual(visibleBoundary.listTop - 1);
        expect(row.bottom).toBeLessThanOrEqual(visibleBoundary.listBottom + 1);
      }
    }

    await chooser.getByRole("checkbox", { name: "OpenCode" }).click();
    await chooser.getByRole("button", { name: "Save Agent choices" }).click();
    ...
    await agentsWorkspace.getByRole("button", { name: "Choose Agents" }).click();
    const allAgentsChooser = page.getByRole("dialog", { name: "Choose Agents" });
    const checkedAgents = allAgentsChooser.getByRole("checkbox", { checked: true });
    while (await checkedAgents.count()) await checkedAgents.first().click();
    await allAgentsChooser.getByRole("button", { name: "Save Agent choices" }).click();
    await expect
      .poll(async () => {
        const settings = await readJson<{ enabledTargetIds?: string[] }>(
          join(appDataRoot, "settings.json")
    ...
    await page.reload();
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    await page.getByRole("region", { name: "Agents", exact: true })
      .getByRole("button", { name: "Choose Agents" }).click();
    const reopenedChooser = page.getByRole("dialog", { name: "Choose Agents" });
    await reopenedChooser.getByRole("checkbox", { name: "OpenCode" }).click();
    await reopenedChooser.getByRole("button", { name: "Save Agent choices" }).click();
  ...
  it("persists a custom Agent command and uses it for the next probe", async () => {
    ...
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    const agentsWorkspace = page.getByRole("region", { name: "Agents", exact: true });
    await agentsWorkspace.getByRole("button", { name: "More actions for OpenCode" }).click();
    ...
    for (const viewport of [
      ...
      await resizeAppWindow(page, viewport.width, viewport.height);
      await expectNoHorizontalOverflow(page, [".app-shell", ".agent-advanced-dialog"]);
    }
    ...
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    await page.getByRole("region", { name: "Agents", exact: true })
      .getByRole("button", { name: "More actions for OpenCode" }).click();
    await page.getByRole("menuitem", { name: "Advanced setup" }).click();
    await expect
  ...
  it("switches, persists, and contains all supported interface languages", async () => {
    ...
    await page.getByRole("button", { name: "Agents", exact: true }).click();
    await page.getByRole("button", { name: "选择 Agents", exact: true }).waitFor();
    expect(await page.getByRole("button", { name: "Choose Agents", exact: true }).count()).toBe(0);
    await expect
  ...
  it("persists the preferred conversation terminal", async () => {
    ...
    await openSettingsCategory(page, "Conversations");
  ...
  it("opens a Skill update preview from the Profile status lane", async () => {
    ...
    await page.getByRole("button", { name: "Profile Skill actions", exact: true }).click();
    await page.getByRole("menuitem", { name: "Check Profile Skill updates" }).click();
  ...
  it("keeps mixed Profile Skill status and action lanes aligned", async () => {
    ...
    const skillManager = page.getByRole("region", { name: "Profile Skills" });
    const checkSkillUpdates = page.getByRole("button", {
      name: "Profile Skill actions"
    });
    const addSkill = page.locator('[data-profile-composer-id="skills"]').getByRole("button", { name: "Add Skills", exact: true });
    ...
    await setComposerResourcePolicy(page, "Skills", "OpenCode", "Keep Agent");
    await expect.poll(() => newRow.count()).toBe(0);
    await expect.poll(() => skillManager.getByText("Current state unavailable").count()).toBe(0);
    await expect.poll(() => skillManager.getByRole("switch").count()).toBe(0);
    expectAlignedResourceRows(
      await readAlignedResourceRows(page.locator(".profile-skill-row")),
      { minimumRows: 2 }
    );
  ...
  it.each([0, 15])("applies a Profile Skill Group gate and restores its saved member choices with %ipx scrollbars", async (scrollbarWidth) => {
    ...
    await expect.poll(async () => {
      ...
      return profile.resources.skillGroups?.[0]?.enabled;
    }).toBe(false);
    await previewAndApply(page, "OpenCode");
    const targetSkills = join(root, "home", ".config", "opencode", "skills");
    await expect(fileExists(join(targetSkills, "ui-alpha-skill"))).resolves.toBe(false);
    await expect(fileExists(join(targetSkills, "shared-reviewer"))).resolves.toBe(false);

    await group.getByRole("switch", { name: "Turn on Review pack" }).click();
    await saveProfile(page);
    await previewAndApply(page, "OpenCode");
    await expect(readFile(join(targetSkills, "ui-alpha-skill", "SKILL.md"), "utf8"))
      .resolves.toContain("alpha skill prompt");
    await expect(readFile(join(targetSkills, "shared-reviewer", "SKILL.md"), "utf8"))
      .resolves.toContain("Shared Reviewer");
    const persisted = await readJson<{
      skillGroups: Array<{ enabled: boolean }>;
  ...
  it("keeps the shared desktop visual contract stable across workspaces and review surfaces", async () => {
    ...
    const profileContextGeometry = await page.locator(".profile-hero__title").evaluate((group) => {
      ...
      const switcherBox = group.querySelector(".profile-switcher")!.getBoundingClientRect();
      return { controlsAreContained: switcherBox.left >= box.left && switcherBox.right <= box.right + 1 };
    });
    ...
    const workspaceContextGeometry = await page.locator(".project-detail__title-control").evaluate((group) => {
      const box = group.getBoundingClientRect();
      const switcherBox = group.querySelector(".project-switcher")!.getBoundingClientRect();
      return { controlsAreContained: switcherBox.left >= box.left && switcherBox.right <= box.right + 1 };
    });
    ...
    expect(profileContextGeometry).toEqual({ controlsAreContained: true });
    ...
    expect(workspaceContextGeometry).toEqual({ controlsAreContained: true });
    expect(workspaceActionTopology).toEqual({
      addInPageHeader: false,
      refreshInActions: false
    });
    ...
    expect(targetListGeometry.identityWidth).toBeLessThanOrEqual(400);
    ...
    expect(await compactActionsHeader.count()).toBe(0);
    ...
    const cleanupDrawerGeometry = await page.locator(".library-drawer").evaluate((drawer) => {
      ...
      const pageActionStates = Array.from(
        document.querySelectorAll<HTMLButtonElement>(".library-page-header .ui-page-header__actions button")
      ...
      return {
        ...
        pageActionsDisabled: pageActionStates.length > 0 && pageActionStates.every(Boolean),
        radius: drawerStyles.borderRadius,
        shadowMatches: drawerStyles.boxShadow === expectedShadow
      };
    });
    expect(cleanupDrawerGeometry, JSON.stringify(cleanupDrawerGeometry)).toEqual({
      actionHeights: [32, 32],
      borderMatches: true,
