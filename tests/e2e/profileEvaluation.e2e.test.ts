...
printf '{"type":"text","part":{"text":"profile=%s skill=%s isolated=%s project-resources-excluded=%s"}}\\n' "$profile_loaded" "$skill_loaded" "$home_isolated" "$project_resources_excluded"
...
`, "utf8");
      ...
      await evaluationDialog.getByText(
        "profile=true skill=true isolated=true project-resources-excluded=true"
      ).waitFor();
      ...
      expect(stored).toMatchObject({
        targetId: "opencode",
        finalResponse: "profile=true skill=true isolated=true project-resources-excluded=true",
