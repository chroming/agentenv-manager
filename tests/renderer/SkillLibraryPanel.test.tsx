...
describe("SkillLibraryPanel", () => {
  ...
  it("keeps the skill list clean and routes secondary workflows through drawers and row actions", async () => {
      ...
      <LegacySkillLibraryPanel
        ...
        skillInventory={[
          ...
          },
          {
            ...
            contentHash: "external-cli-skill-hash",
            contentMatchesLibrary: true,
            locationRole: "compatibility-runtime",
            sharedLocation: true,
            unmanagedLocationId: "unmanaged-external-cli-skill-shared",
            unmanagedCoverage: "exact"
          },
          {
            id: "external-cli-skill",
    ...
    const githubSourceDetails = screen.getByLabelText("Full source for github-reviewer");
    expect(githubSourceDetails).not.toHaveTextContent(/Source updated/);
    fireEvent.mouseEnter(githubSourceDetails);
    expect(await screen.findByRole("tooltip")).toHaveTextContent(/Source updated.*7\/18\/2026/);
    ...
    fireEvent.mouseLeave(sharedIdentity);
    await waitFor(() => expect(screen.queryByRole("tooltip")).not.toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.queryByRole("tooltip")).not.toBeInTheDocument());
    expect(sharedIdentity).not.toHaveAttribute("title");
    ...
    const updateReview = updateStatus.closest("button");
    expect(updateReview).toHaveAccessibleName("Review update github-reviewer");
    ...
    expect(onPreviewLibrarySkillUpdate).toHaveBeenCalledTimes(3);
    fireEvent.click(within(sharedRow).getByRole("button", { name: "More actions for shared-reviewer" }));
    ...
    const partiallyKeptGroup = screen.getByRole("group", {
      name: "Cleanup group external-cli-skill"
    });
    expect(
    ...
    await waitFor(() => expect(onCopyCleanupDetails).toHaveBeenCalledWith(
      expect.stringContaining("Path: /tmp/trae/skills/debug-helper")
    ));
    ...
    await waitFor(() => expect(onAutoConsolidateSkillGroups).toHaveBeenLastCalledWith([
      expect.objectContaining({
        skillKey: "debug-helper",
        locations: [expect.objectContaining({ path: "/tmp/trae/skills/debug-helper" })]
      })
    ]));
    await waitFor(() =>
      expect(screen.queryByRole("dialog", { name: "Skill details debug-helper" }))
        .not.toBeInTheDocument()
    );
    fireEvent.click(
      within(conflictGroup).getByRole("button", {
