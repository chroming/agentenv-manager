...
`
  ...
  it("finds managed installs without traversing unrelated cyclic skill content", async () => {
    ...
    const managedInstall = join(skillsDir, "managed-sample");
    const cyclicSkill = join(skillsDir, "cyclic-sample");
    await mkdir(managedInstall, { recursive: true });
    await writeFile(join(managedInstall, "SKILL.md"), "# Managed sample\n", "utf8");
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        targetId: "codex",
        kind: "skill",
        source: "skills-library/managed-sample"
      })}\n`,
      "utf8"
    );
    await mkdir(cyclicSkill, { recursive: true });
    await writeFile(join(cyclicSkill, "SKILL.md"), "# Cyclic sample\n", "utf8");
    await symlink(cyclicSkill, join(cyclicSkill, "cyclic-sample"), "dir");

    const store = createSkillLibraryStore(paths);
    await expect(
      store.findManagedInstallPaths("managed-sample", [
        {
          targetId: "opencode",
