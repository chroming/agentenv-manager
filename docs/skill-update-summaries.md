...
- 默认展示结论与分类重点，风险优先。说明 Agent 在什么任务中会有什么不同，需要用户采取什么行动；技术术语和文件名不能替代实际影响。必要的使用影响紧随变化展示，文件依据、旧摘要的额外条目和生成元信息放在 Details 内，原始内容不丢弃；未覆盖范围仍明确提示。
- 摘要和分析复用无横向内边距的 ResourcePanelToolbar，标题与内容共用起点。独立 Profile 分析只使用一层 DialogBody；菜单使用统一 Sparkles 图标。建议标签的“新标签”位于 TagChip 外，不属于标签值。
