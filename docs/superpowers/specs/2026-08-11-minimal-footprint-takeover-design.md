...
Legacy `.agentenv-owner.json` files remain read-only migration evidence. New deployment paths do not create them. Validity requires a recognized owner plus the observed Target and resource kind; link/copy topology alone never proves legacy state. A fresh explicit Apply or the device-wide Local Skills Manager migrates valid evidence into Target state and removes the old marker through the same Backup transaction. The migration preserves the current Skill bytes, timestamps, and link/copy materialization even when the current deployment preference differs. Missing markers require no user-visible migration; invalid or conflicting markers return to ordinary review.
...
Apply Preview opens the same device-wide Local Skills Manager and may focus the relevant Skill. Closing it returns to that Profile and regenerates Preview from current disk state. The manager never introduces Profile- or Agent-scoped mutation modes.
...
| Layer | Evidence |
|---|---|
| Domain | Telemetry consent gate; markerless deploy; central receipt scan; exact-copy adoption; legacy sidecar migration preserving topology; update propagation; stale and rollback tests |
| Renderer | First-run disclosure, opt-out, session dismissal, Apply local-footprint summary |
| Desktop | First-run ordering, legacy marker notice to Local Skills Manager, Profile Apply, restart persistence, minimum and large viewport Preview |
| Persistence | Target-state receipt survives restart; no marker is created; migrated marker is removed while its Skill topology remains unchanged; previous telemetry opt-out survives migration |
