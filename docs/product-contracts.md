...
- Interface typography uses four semantic weights only: regular `400` for body and metadata, medium `500` for controls and routine navigation, semibold `600` for object identity and current selection, and heading `650` for first-level page titles. Routine status, badges, buttons, labels, and metadata MUST NOT become bold merely to compensate for weak grouping; interface CSS MUST NOT use weights above `650`.
