...
- The shared area exposes two stable policies and one separate migration command. `Leave unchanged` is the implicit default, remains read-only, and creates no management receipt. `Manage in place` imports or reuses Library content, preserves the shared path and topology, records ownership only in AgentEnv data, and does not edit Profiles or create Agent-private copies. `Move and remove shared copies` is a reviewed migration transaction, not a symmetric policy toggle: it prepares supported Agents from their saved Profiles, verifies every destination, and removes shared copies last.
- Shared review MUST NOT outrank first Agent setup. An unclaimed or `Leave unchanged` shared directory may appear as quiet supporting status and remains available in Local Skills Manager, but it does not create a startup action. AgentEnv escalates it only after the user chooses `Manage in place` or `Move and remove shared copies`, or when an Apply result needs a concrete shared-path decision.
...
- `Move and remove shared copies` MUST be explicit and warning-level wherever it appears. Profile editing remains available while a shared runtime is active, but it edits only the intended post-migration state; rows show `After move`, never false `Apply pending`, and the Profile and Apply Preview expose the same direct migration route. Migration Preview lists supported consumers, Profile outcomes, exact shared paths, unsupported-Agent risk, and every destination. The transaction creates one restorable backup for the shared state, shared paths, affected Agent paths, and Agent state; prepares destinations first, removes only the listed shared entries last, verifies the result, and restores the whole transaction on any failure. Linked source folders and the shared parent directory are never removed. After migration the surface shows the resulting Profiles-only state and a direct route to its restore points; switching a compact policy control MUST NOT pretend to recreate the removed shared copies.
...
The surface owns filesystem-copy normalization into Library and shared-area intent. Ordinary management does not edit Profile membership or orchestrate Apply. Only the explicit `Move and remove shared copies` command may coordinate saved Profile intent across Agents. Scan itself is read-only: it produces a plan, and mutation begins only after the user confirms that plan.
...
- `~/.agents/skills` and other shared runtime roots are special. Their stable policies are `Leave unchanged` and `Manage in place`; `Move and remove shared copies` is a separate migration command. Ordinary per-Agent commands MUST NOT silently choose a policy or run the migration. Device-wide `Manage eligible` may perform only the non-moving, in-place management outcome.
...
- Managing a shared Skill keeps one shared runtime copy active, removes reviewed redundant Agent-specific copies, and writes only a central device receipt. `Move and remove shared copies` performs the separate backed-up, verified cross-Agent switch without deleting Library content.
