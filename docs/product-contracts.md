...
- Import presents only `Local folder` and `Repository` as source modes. A local selection MAY be one Skill directory or a containing directory with multiple nested Skills; a separate Projects workflow or saved-location concept MUST NOT duplicate this intent.
...
- The global auto-check setting controls scheduling only. Within that schedule, only source groups whose `Automatic checks` policy is enabled are read; results are surfaced only for member Skills whose independent policy is `Tracked`.
...
- Repository source groups default `Automatic checks` on; local source groups default it off. The group-level switch controls only startup and scheduled source reads. Manual `Check` and `Check all` always remain available. The per-Skill `Track updates` setting remains independent and controls whether a discovered change becomes an update reminder or participates in batch Update review.
- `Refresh` reloads the current source projection and cached observation without contacting the source. `Check` and `Check all` explicitly read source repositories or local folders. A missing local source reports an unavailable source while retaining Library content and the last complete observation. Switching between `Skill list` and `By source` MUST preserve the mounted view state and MUST NOT flash an empty Library.
...
- A merged source preserves `Automatic checks` when all participating groups agree. Mixed settings are disclosed in Preview and resolve to off; merging into an existing destination includes that destination in this decision.
