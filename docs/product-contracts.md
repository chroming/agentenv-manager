...
- Expanded Profile Skills and MCP editors share one alignment contract: the toolbar title starts on the same identity lane as every resource name, row controls end on the same action lane, and the editor owns one consistent horizontal inset. At the supported minimum width, a Skill's update or relink command collapses to the shared `32px` icon command with an accessible name while identity, lifecycle state, policy, and overflow retain dedicated non-overlapping lanes.
...
- Any compact file diff may open the shared read-only diff workspace. The workspace keeps a tree of all changed files on the left and the selected true diff on the right, supports a keyboard-adjustable pane divider plus normal-window resize and in-app maximize, and owns no mutation command. While open it suspends the parent decision dialog; `Escape` closes only the workspace, restores focus to its invoking control, and leaves the reviewed Apply, Import, Update, or Merge decision intact.
