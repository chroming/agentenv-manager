...
- In `Automatic` connection mode, a GitHub API 401, 403, or private-repository-style 404 MAY retry through System Git and the equivalent GitHub SSH transport. The scan summary MUST disclose `SSH fallback`, and the imported source remains a Git source with its canonical HTTPS locator. Rate limits, network failures, malformed URLs, and unknown providers MUST NOT trigger this fallback. When automatic fallback cannot proceed, the user MAY explicitly choose `Try with System Git`.
