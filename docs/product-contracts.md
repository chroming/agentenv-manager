...
- Skill Library and Profile list rows expose their existing overflow command set through the operating system's native desktop context menu. Right-click MUST target the row under the pointer without inventing additional commands, changing command availability, bypassing dirty-state or destructive confirmation, or silently applying the command to the currently selected row instead. The native menu owns platform geometry, keyboard dismissal, viewport placement, and focus return; AgentEnv MUST NOT imitate it with a card-like renderer popover.
