...
- Selecting a Library Skill opens a read-only file browser. It lists only regular files contained by the canonical Library Skill, hides AgentEnv metadata, skips links, rejects escaped or resolved-outside paths, previews text with syntax highlighting, and reports binary or oversized files without decoding them. Browsing never changes the Skill, its source, a Profile, or an Agent install.
...
- A globally disabled Skill that is still active only through an unmanaged or externally managed copy remains outside AgentEnv authority. Preview MUST disclose that preserved copy without blocking unrelated Profile changes, and Apply MUST NOT delete or take ownership of it.
