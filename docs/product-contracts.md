...
- Skill Library and Profile list rows expose their existing overflow command set through the desktop context-menu gesture. Right-click MUST target the row under the pointer without inventing additional commands, changing command availability, bypassing dirty-state or destructive confirmation, or silently applying the command to the currently selected row instead. The context menu is keyboard-dismissible, constrained to the viewport, and returns focus to the owning row action.
