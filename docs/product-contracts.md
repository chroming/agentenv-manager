...
Managed copy is the default for new installations. When an existing installation first runs a version with this default, AgentEnv MUST ask once whether to use Managed copy or retain Live link. The choice changes only the deployment preference: existing Agent paths remain byte-for-byte unchanged until a later Profile Preview and Apply explicitly converts them. Deferring the choice lasts only for the current launch. Legacy `auto` resolves to Managed copy, but the migration notice remains required so the lack of immediate filesystem changes is not mistaken for a failed upgrade.
...
- `Ready to manage` includes: one writable outside copy, identical writable outside duplicates, an unimported shared compatibility group with one unambiguous content version, an imported shared compatibility copy whose current Agent use can be preserved by the management workflow, copies already matching the canonical AgentEnv copy, prepared shared-folder migrations, and safely removable broken symbolic links including links in shared compatibility roots. Changed managed copies and local copies that differ from the canonical copy remain in `Needs your decision`. Generic bulk-ready rows expose only Details and secondary retention controls in overflow; they MUST NOT repeat a primary row action. A prepared shared-folder migration also exposes its dedicated `Move` action for users who want to handle it independently.
...
- Local Skills Manager is the only workflow that resolves existing Agent copies. The entry may be scoped to the current Profile, current Agent, all local Skills, or a shared compatibility location; every scope filters the same inventory and uses the same preview, transaction, backup, rollback, ignore, collection, and recovery owners.
...
- `Manage eligible` remains available for users who want broad takeover. It MUST exclude any item requiring a content/version choice or a shared-folder policy choice.
...
- `~/.agents/skills` and other shared compatibility roots are special. They may affect several Agents and therefore MUST remain a shared decision: keep outside AgentEnv, or prepare every known consumer and then move the shared copy. Ordinary per-Agent one-click management MUST NOT silently consume them.
...
- A shared Skill not yet in AgentEnv follows the same save-and-manage intent as every other local Skill. One content version is eligible for the confirmed `Manage N` plan; multiple different content hashes remain in `Needs your decision`, show the number of different versions in the row, and add version choice inside the review flow.
...
- AgentEnv storage is the only canonical managed Skill store, and Target-specific Skill directories are the normal deployment locations. A compatibility root such as `~/.agents/skills` is never a completed AgentEnv deployment destination. Once its shared Skill has one unambiguous managed version, Local Skills Manager places it in `Ready to manage`; internal Agent preparation is a conditional prerequisite of that command, not a separate user decision. The confirmed operation preserves the current Profile intent by updating each healthy active Profile or capturing current Skills into a new Skills-only Profile for an unmanaged Agent, then moves or removes the shared copy according to current managed Skill and Profile policy. `Leave shared location unmanaged` remains the explicit opt-out in overflow. Automated preparation MUST leave Instructions and MCPs unchanged, refuse to absorb unrelated pending, drifted, or recovery-required Profile changes, and keep the shared copy active after any partial failure so the command can be retried safely. The manager MUST NOT show per-Agent `Needs Apply` chips or expose internal preparation records as commands.
...
- After every affected Agent has an explicit decision for the current managed Skill ID and exact current shared-path set, the row moves to `Ready to manage` and shows `Ready` / `Move`; either `Move out of shared folder` or the reviewed `Manage N` batch may cross the mutation boundary. The manager revalidates the current saved Profile's Skill mode, install-or-omit intent, destination name, managed Skill identity, and exact shared paths immediately before mutation. If only this Skill's saved intent changed after preparation, the confirmed command MUST absorb that scoped intent in the same backup transaction instead of sending the user to Profiles to Apply again. Instructions, MCPs, metadata, and other Skills MUST remain untouched and pending; a different shared copy, `Keep current` Skills policy, recovery state, or unsafe destination still stops the transaction before mutation.
...
retains the same migration-only authority used by other consumers: ordinary Apply deploys only
to Pi's dedicated Skills directory, while shared cleanup remains a separately reviewed,
cross-Agent operation. Pi has no built-in MCP configuration; extension-provided protocols MUST
...
6. When shared Skills require normalization, Review opens Local Skills Manager scoped to those
...
- Adding a shared compatibility Skill to Library keeps one shared runtime copy active and removes redundant Target-specific copies. Apply prepares each installed consumer without creating duplicate runtime copies; `Move out of shared folder` then performs one backed-up, verified cross-Agent switch without deleting Library content.
