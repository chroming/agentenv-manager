...
- `Keep Agent` gives the complete resource header a quiet neutral treatment so the non-managed category is visible without reading the policy label. The disclosure and policy selector remain fully legible and interactive; the row MUST NOT use whole-container opacity or disabled styling because its saved Profile content remains editable.
