...
- Disabling a Skill removes an AgentEnv-owned deployment automatically. An ordinary unmanaged copy at that exact Target destination MAY be removed only as a reviewed, backed-up Apply effect. A copy owned by another manager, protected by an Ignore rule, or active from another unmanaged runtime location remains blocking because AgentEnv cannot safely promise that the Skill will be disabled.
...
conflict and MUST NOT be admitted by the reviewed replacement plan.
...
- When an online source exposes a commit time for the Skill subtree, AgentEnv stores it separately from the local Library write time. The Skill list and duplicate-import comparison label this as the upstream update time; unavailable source time is omitted rather than inferred from local file timestamps.
...
- A source group is identified by one deterministically normalized complete import scope: repository identity, ref, and directory. Repository roots, different refs, parent and child directories, and unrelated complete links MUST remain separate. Similarity, URL prefixes, shared hosts, Skill names, and matching content MUST NOT merge groups.
- Repository import records the reviewed source scope and each imported Skill's relative source path. Legacy online Skills without collection metadata appear as exact one-Skill source groups without rewriting their files or metadata.
...
- `Refresh` reloads the current source projection and cached observation without contacting the repository. `Check` and `Check all` explicitly contact source repositories. Switching between `Skill list` and `By source` MUST preserve the mounted view state and MUST NOT flash an empty Library.
...
- Row status badges use a compact, non-truncating vocabulary: `Managed`, `N versions`, `Changed`, `External`, `Shared`, `Ready`, `Kept`, `Unavailable`, and `Ignored`. A content conflict without a Library canonical MUST state the number of versions instead of exposing the generic internal state `Conflict`. The selectable hover/focus detail carries the complete explanation; a badge MUST NOT clip or ellipsize its visible label.
...
- `Ready to clean up` includes: one unmanaged copy, identical unmanaged duplicates, an unimported shared compatibility group with one unambiguous content version, copies already matching Library, stale managed copies, local copies that differ from an existing Library canonical copy, and unowned broken symbolic links including links in shared compatibility roots. Generic bulk-ready rows expose only Details and secondary retention controls in overflow; they MUST NOT repeat a primary row action. A prepared shared-copy replacement is the exception: it exposes the dedicated `Replace shared` action and is not part of `Clean up N`.
...
- An unowned broken symbolic link can be planned for removal because cleanup removes only the link itself and backs up its link metadata. A link recorded by an external manager is also eligible only when ownership evidence explicitly reports `broken-link`; the UI presents it as `Unavailable`, while retaining the stale provenance for diagnostics. An unreadable directory or manifest, a healthy or unknown externally owned path, an unknown Target, a permission error, or ambiguous canonical content MUST remain in `Needs your decision` and MUST NOT be deleted automatically.
- `Clean up N` is the one emphasized bulk command and appears only when at least one safe plan exists. Its confirmation groups effects as add-and-link, link-to-Library, backup-and-link, repair-link, and remove-unavailable-link, and states that each Skill is backed up independently. A failure in one Skill does not roll back completed independent Skills, and the result reports both completed and remaining groups.
- Decision rows expose one lightweight current action. Read-only details, ignore, shared retention, and restore-to-review controls live in overflow. Internal states such as `Auto-ready`, `Take over`, and `Resolve conflict` MUST NOT be presented as user actions.
...
- Cleanup MUST leave every Profile resource reference byte-for-byte unchanged. A later Apply independently decides whether a managed Skill is installed, omitted, or disabled for that Target; ignored and externally owned paths remain untouched unless the user explicitly resolves a path conflict.
...
- After every affected Target has an explicit current decision, the row moves to `Ready to clean up` and shows `Ready` / `Replace shared copy`; the dedicated confirmation remains the mutation boundary and the row is not included in generic bulk cleanup.
...
- Skill source work is scoped by command: `Check updates`, one Skill's `Review update`, bulk `Review updates`, one source check, and `Check all` never borrow each other's spinner. The active command state survives workspace navigation and restores on return until the operation completes.
...
- Interface typography uses four semantic weights only: regular `400` for body, controls, routine navigation, status, and metadata; medium `500` for the single identity anchor in a repeated row and the current navigation item; semibold `600` only for dialog titles, the selected primary object, and the brand; and heading `650` for first-level page titles. Native `strong` and `b` elements inherit by default because semantic markup does not grant visual emphasis to layout values. Routine status, badges, buttons, labels, counts, sources, versions, and timestamps MUST NOT become bold merely to compensate for weak grouping. Interface CSS MUST NOT use weights above `650`, and the product-wide semibold declaration budget is enforced by the style audit.
...
- Skills quick tabs are limited to `Enabled`, `Updates`, and `Disabled`. `Enabled` is the default and excludes globally disabled Skills; Source, Usage, and Agent-install filters live in one on-demand filter region, while `Review updates` is rendered only when one or more updates can actually be reviewed. Applying those reviewed candidates remains a distinct confirmation command inside the dialog.
