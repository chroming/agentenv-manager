...
- Expanded Skill Groups use a neutral nested-list treatment: the group header remains stable, member rows are visibly indented without a colored panel or connector line, and low-frequency membership commands stay in each member's contextual menu. In Library, selecting a member name opens its existing read-only Skill browser and `Remove from group` changes only that reusable group. In Profile, grouped members cannot be removed individually. Profile Groups use the shared disclosure's lightweight list appearance; Group, member and direct Skill switches share one action rail. The enclosing Skills panel owns scrolling, not each expanded Group. Every member remains keyboard- and pointer-reachable at the minimum supported window size.
- A Profile Skill Group has one independent On/Off gate. When all references gate a Skill Off, it is omitted from the effective Profile regardless of its saved member preference; member switches display the effective Off state and are read-only, while saved preferences remain unchanged. Turning the group On restores those saved member choices. Apply, Compare, evaluation, deployment fingerprints, update checks, and summaries MUST consume the same flattened effective Skill set. Under `Use Profile`, disabled or gated-off Skill identities are absent from the deployment fingerprint, so deleting an already-Off Group is a Target no-op and cannot create false Apply-pending state. A grouped member cannot be detached from that group individually; removing the group removes group-only references while preserving Skills that were also added directly or belong to another group.
...
- AI update summaries, duplicate comparisons, Profile reviews and run interpretations share a single-column decision brief: one conclusion, at most three ordinary findings initially visible, and concrete risks retained visibly. Evidence links, metadata, coverage and additional ordinary findings belong in an explicit details disclosure. Never truncate cached findings or regenerate them automatically to meet a newer presentation preference. Regeneration belongs to the analysis heading, not between prose and evidence. Prompts prioritize behavior, consequences and specific actions rather than inventories or repeated generic advice.
- An inline error containing a diagnostic reference MUST offer an adjacent icon-only Copy details action, including inside modals. It copies the redacted diagnostic record when available and falls back to the displayed error plus reference when lookup fails. Clipboard failure remains locally retryable; ordinary notices without a diagnostic reference do not gain a copy control.
- Skill change analysis treats invalid or dangerous Skill content as evidence to analyze, not instructions to execute or grounds to reject analysis. A completed, structurally valid model response with missing or invalid file citations remains readable and persistable: retain its findings, link only exact supplied evidence, and label affected findings as unverified. This limitation persists across cache/restart. Do not silently drop security findings, invent evidence, or automatically spend quota on retries. Transport errors, cancellation, incomplete output and unparseable responses remain genuine generation failures and preserve the previous summary.
...
- Catalog resource rows use the shared compact `42px`, default `58px`, and comfortable `64px` minimum-height tokens. Single-line catalog records use compact density; two-line source groups retain default density. The Profile composer header has its own `52px` shell token. These are reading-unit variants, not mandatory fixed heights for wrapped content; control hit areas and body typography do not shrink to force a fit.
- Information removed from the default projection MUST retain a discoverable destination in existing hover details, object details, or decision previews. Identity, actionable failures, and mutation consequences remain visible where needed; fewer words or more icons alone do not establish clarity.
- ResourcePanelToolbar owns spacing for both Profile and Workspace expanded content. Short content has no forced blank height; long lists have bounded scrolling, a stable scrollbar gutter, and a visible continuation affordance. Scrolling works over nested rows, not only group headers. Source membership uses disclosure and indentation without repeating the child table header.
- DetailList owns optional commands beside their corresponding values. Paths, timestamps, hashes, and reference details remain selectable. Instruction list descriptions and reference counts do not compete on one short metadata line; complete usage remains in the detail surface. Compact list/detail panes use a bounded adaptive list width rather than a fixed-width squeeze.
- Settings sections fill the available content width; commands align to that content edge and fields retain their own control widths. Idle bulk-update counts belong in the dialog header, active queue progress appears after execution starts, and failed previews retain visible reasons. Empty AI result sections do not repeat the generation command as a heading. These presentation changes do not alter update selection, cancellation, recovery, or deployment behavior.
- Settings single-line preferences use a `48px` minimum row height; rows retaining a description use `64px` and may grow for wrapping. Section vertical padding is `8px`. Removing a description also removes its density budget, without shrinking controls or stretching sparse content to fill the window.
- Standard toolbar, form and dialog controls share the `32px` height. Primary emphasis changes tone, not height; the prominent and dialog height tokens alias the standard token. Compact local commands use `28px`. Inline utility icons use IconButton's `inline` appearance: a `14px` glyph inside a compact hit area, with the same busy slot and hover/focus behavior. Page styles MUST NOT shrink these controls to make the glyph look quieter. Segmented controls include their border and inset in their declared outer height. Navigation tabs, switches, metadata and resource artwork retain their distinct semantic sizes.
- Skill source text uses the same leading text origin and vertical center for local text, HTTPS links, and SSH copy actions. Interactive source text uses TextAction rather than inheriting full button centering or minimum height. Geometry evidence measures the actual source text against its cell, not only the outer grid tracks.
- Layout regression fixtures MUST mix unlike rendering branches in the same projection: plain text and clickable text, local and online sources, short and overflowing content, routine and actionable statuses. Hover and focus MUST NOT move the initiating control or resize its row. Working states preserve the status icon slot and prevent duplicate submission. Component tests establish state semantics, not pixel alignment; Electron measurements establish geometry, not visual hierarchy.
- Changes that remove default text MUST re-evaluate the owning density variant and test retained-description rows separately. Sparse settings MUST NOT inherit space reserved for removed descriptions; wrapped errors and expanded settings MUST retain readable growth rather than be forced into the compact row height.
...
- The Agents Profile lane is a single reading line at every supported width, for both local and
  SSH Agents: the Profile name or current command, with a shared status hint for pending changes,
  drift, or local overrides. Routine applied-state text is available with the Profile detail
  rather than repeated underneath. Recovery keeps its explicit Open Recovery command. Local
...
- Skill catalog status headings align with status text, reserving the same leading icon slot as rows. Source group and expanded Skill actions share the trailing edge; status text stays vertically centered in tagged and untagged rows. Geometry checks measure text and buttons, not only their containing columns.
- Opening a modal focuses its initial control without opening that control's tooltip. Subsequent keyboard focus and deliberate pointer hover still expose the control's hint; the close button and Escape remain available.
...
- Enabling History sources starts indexing without making the conversation workspace inert or
  covering its controls. Before the first result, progress lives inside the empty list. Search
  and the History sources menu remain usable while indexing; only repeated Refresh is disabled.
  Verify this with an unfinished scan, not merely after results arrive.
...
- Resource child indentation belongs to the shared disclosure pattern, not page overrides: direct
  Profile and Workspace members share one inset; nested Group members add one depth step without
  moving the state/action rail. Source candidates also add one depth step below their source identity,
...
- At minimum width, the Skill identity column takes priority over source and tag metadata; common
  names retain room without shrinking type or hiding actionable status. Agent identity columns stop
